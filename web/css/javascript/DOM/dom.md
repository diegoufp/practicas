...
```js
// en este tema solo se modifico el codigo del js
const cards = document.getElementById('cards');
const items = document.getElementById('items');
...
const pintarCarrito = () => {
    ...
    // una vez que terminamos de pintan el carrito ahora podemos pintar el footer
    pintarFooter();
};

const pintarFooter = () => {
    footer.innerHTML = ''; //Tenemos que reiniciar el footer para que no se sobreescriba nuestra informacion
    // tenemos que preguntar si no contiene elementos
    if(Object.keys(carrito).length === 0){
        footer.innerHTML = `<th scope="row" colspan="5">Carrito vacío - comience a comprar!</th>`
        return
    }

    // creamos una constante de la suma de todas las cantidades
    // para usar reducer tenermos que utilizar Object.values del carrito por que e suna coleccion de objetos y no podemos usar todas las funciones del array amenos que utilicemos Object.values
    // reducer toma una funcion de flecha y esta funcion tiene que retornan algo, nosotros vamos a recorrer todos elemenos de esta collecion de objetos
    // en tonces vamoa a tener un acumulador (acc) y nosotros vamos a necesitar acceder a la cantidad
    // el acc va a ir acumulando por cada itenacion que hagamos como suma
    // lo que vamos a devolver es un numero por eso le vamos a poner una coma y un cero
    const nCantidad = Object.values(carrito).reduce((acc, {cantidad}) => acc + cantidad, 0);
    const nPrecio = Object.values(carrito).reduce((acc, {cantidad, precio}) => acc + cantidad * precio, 0);

    // ahora que tenemos el acumulador tenemos que pintarlo en nuestro template footer
    templateFooter.querySelectorAll('td')[0].textContent = nCantidad;
    templateFooter.querySelector('span').textContent = nPrecio;

    // una vez que tenemos nuestro template tenemos que clonarlo
    const clone = templateFooter.cloneNode(true);
    fragment.appendChild(clone);
    // como esto no es un ciclo, lo agregamos directamente a donde tenemos el footer
    footer.appendChild(fragment)

    // ahora implemetaremos la funcionalidad de vaciar carrito
    const btnVaciar = document.getElementById('vaciar-carrito');
    btnVaciar.addEventListener('click', () => {
        carrito = {};// con esto se limpiaria el carrito 
        // ahora lo tenemos que volver a pintar
        pintarCarrito();
    });
};
```

### Evento aumentar y disminuit cantidad

```js
//en este caso se modifico solamente el codigo de js
const cards = document.getElementById('cards');
const items = document.getElementById('items');
const footer = document.getElementById('footer'); 
const templateCard = document.getElementById('template-card').content;
...
// creamos esto para la funcionalidad de los botones aumentar y disminuir cantidad
items.addEventListener('click', e => {
    btnAccion(e);
});
...
const btnAccion = e => {
    ...
    if(e.target.classList.contains('btn-danger')){
        const producto = carrito[e.target.dataset.id];
        producto.cantidad--;
        // con esto cuando la cantidad del producto llege a cero lo estariamos borrando del carrito
...
### LocalStorage
Nos damos cuenta que si actualizamos el sitio web desaparece el carrito de compras y eso no es optimo.
para guardar los productos en el localStorage tenemos que ver en que parte vamos a guardar los articulos.
```js
// solametne se corrige el codigo de js
const cards = document.getElementById('cards');
const items = document.getElementById('items');
const footer = document.getElementById('footer'); 
const templateCard = document.getElementById('template-card').content;
...
document.addEventListener('DOMContentLoaded', () => {
    fetchData();
    // una vez que cargo nuestro sitio web le podemos hacer la pregunta del localstorage
...
const pintarCarrito = () => {
    ...
    // para guardar la infomacion le decimos:
    localStorage.setItem('carrito', JSON.stringify(carrito)); //con esto estriamos respaldando nuestro carrito de compras en nuestro localstorage
};
